<template>
  <section class="py-20 mt-10">
    <div class="container mx-auto">
      <div class="flex flex-wrap justify-center gap-10">
        <NuxtLink to="/">
          <div
            class="flex flex-col items-center bg-white dark:bg-gray-800/50 p-6 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300">
            <div class="bg-lightBackground p-5 rounded-full mb-5">
              <img src="/assets/icons/gif/image.gif" alt="3D Assets" class="w-16 h-16" />
            </div>
            <h3 class="text-2xl font-semibold mb-2">Image generators</h3>
            <p class="text-sm text-center">quality picture generate ai</p>
          </div>
        </NuxtLink>

        <NuxtLink to="/">
          <div
            class="flex flex-col items-center bg-white p-6 dark:bg-gray-800/50 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300">
            <div class="bg-lightBackground p-5 rounded-full mb-5">
              <img src="/assets/icons/gif/video.gif" alt="Illustrations" class="w-16 h-16" />
            </div>
            <h3 class="text-2xl font-semibold mb-2">Video generators</h3>
            <p class="text-sm text-center">quality video generate ai</p>
          </div>
        </NuxtLink>

        <NuxtLink to="/">
          <div
            class="flex flex-col items-center bg-white dark:bg-gray-800/50 p-6 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300">
            <div class="bg-lightBackground p-5 rounded-full mb-5">
              <img src="/assets/icons/gif/chat.gif" alt="Illustrations" class="w-16 h-16" />
            </div>
            <h3 class="text-2xl font-semibold mb-2">Chat Bot ai smart</h3>
            <p class="text-sm text-center">smart chat ai</p>
          </div>
        </NuxtLink>

        <NuxtLink to="/">
          <div
            class="flex flex-col items-center bg-white dark:bg-gray-800/50 p-6 rounded-lg shadow-lg hover:shadow-2xl transition-all duration-300">
            <div class="bg-lightBackground p-5 rounded-full mb-5">
              <img
                src="/assets/icons/gif/graphics-software.gif"
                alt="Illustrations"
                class="w-16 h-16" />
            </div>
            <h3 class="text-2xl font-semibold mb-2">Softwares ai smart</h3>
            <p class="text-sm text-center">smart chat ai</p>
          </div>
        </NuxtLink>
      </div>
    </div>
  </section>
</template>
